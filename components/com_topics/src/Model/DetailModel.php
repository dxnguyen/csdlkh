...
class DetailModel extends ItemModel
{
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_topics')) && (!$user->authorise('core.edit', 'com_topics')))
		{
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_TOPICS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_TOPICS_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->department_id) && $this->_item->department_id != '')
		{
			if (is_object($this->_item->department_id))
			{
				$this->_item->department_id = ArrayHelper::fromObject($this->_item->department_id);
			}

			$values = (is_array($this->_item->department_id)) ? $this->_item->department_id : explode(',',$this->_item->department_id);
			...
			$this->_item->department_id = !empty($textValue) ? implode(', ', $textValue) : $this->_item->department_id;
