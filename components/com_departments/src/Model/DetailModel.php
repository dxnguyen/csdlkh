...
use \Departments\Component\Departments\Site\Helper\DepartmentsHelper;
...
class DetailModel extends ItemModel
{
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_departments')) && (!$user->authorise('core.edit', 'com_departments')))
		{
	...
	public function getItem($id = null)
	{
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_DEPARTMENTS_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_DEPARTMENTS_ITEM_NOT_LOADED'), 404);
			}
		}
		...
		if (isset($this->_item->type) && $this->_item->type != '')
		{
			...
			{
				$db    = $this->getDbo();
				$query = "select id, title from #__departments WHERE id = '$value' ";
