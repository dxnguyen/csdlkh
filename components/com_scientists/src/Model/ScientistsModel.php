...
class ScientistsModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		if (!Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_scientists'))
		{
		...
		else
		{
			...
			if (!empty($search))
			{
				...
				else
				{
					...
					$query->where('( a.name LIKE ' . $search . '  OR  a.address LIKE ' . $search . '  OR  a.phone LIKE ' . $search . '  OR  a.email LIKE ' . $search . ' )');
				}
			}
	...
	public function getItems()
	{
		...
		{
				$item->gender = empty($item->gender) ? '' : Text::_('COM_SCIENTISTS_SCIENTISTS_GENDER_OPTION_' . strtoupper(str_replace(' ', '_',$item->gender)));

			if (isset($item->position))
			{
				$values    = explode(',', $item->position);
				$textValue = array();
				...
				$item->position = !empty($textValue) ? implode(', ', $textValue) : $item->position;
			}
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(Text::_("COM_SCIENTISTS_SEARCH_FILTER_DATE_FORMAT"), "warning");
