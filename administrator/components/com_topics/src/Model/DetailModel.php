...
namespace Topics\Component\Topics\Administrator\Model;
// No direct access.
...
class DetailModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_TOPICS';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_topics.detail';
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_topics'))
		{
		...
		foreach ($pks as $pk)
		{
				...
				if (!empty($table->department_id))
				{
					if (is_array($table->department_id))
					{
						$table->department_id = implode(',', $table->department_id);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = $this->getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__topics');
				$max             = $db->loadResult();
